  ...
  for i in range(50000):
   ...
   # print(move[label_id])
    if move[label_id] == "recto rapido":
            ap.set_command(command="forward", speed=55,callback=on_reading)
    if move[label_id] == "recto lento":
            ap.set_command(command="forward", speed=40,callback=on_reading)
    if move[label_id] == "izquierda":
            ap.set_command(command="left", speed=20,callback=on_reading)
    if move[label_id] == "derecha":
            ap.set_command(command="right", speed=20,callback=on_reading)
    t3=time.time()
    if old is not None:
      ...
      tdate=datetime.now()
      filename='resultados/imgs/image_'+ str(tdate.date())+"_%05d_%05d_%05d_%05d_%05d_orig.jpg" % (tdate.hour ,tdate.minute, tdate.second, tdate.microsecond,i)
